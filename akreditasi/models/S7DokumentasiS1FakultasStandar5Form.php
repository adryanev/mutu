...
class S7DokumentasiS1FakultasStandar5Form extends S7DokumentasiS1FakultasStandar5
{
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiS1FakultasStandar5();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiS1FakultasStandar5::find()->select('dokumen')->all();
