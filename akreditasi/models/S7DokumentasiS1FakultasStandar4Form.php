...
class S7DokumentasiS1FakultasStandar4Form extends S7DokumentasiS1FakultasStandar4
{
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiS1FakultasStandar4();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiS1FakultasStandar4::find()->select('dokumen')->all();
