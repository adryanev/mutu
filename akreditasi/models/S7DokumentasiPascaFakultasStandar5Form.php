...
class S7DokumentasiPascaFakultasStandar5Form extends S7DokumentasiPascaFakultasStandar5
{
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiPascaFakultasStandar5();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiPascaFakultasStandar5::find()->select('dokumen')->all();
