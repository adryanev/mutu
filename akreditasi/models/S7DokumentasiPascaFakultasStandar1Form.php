...
class S7DokumentasiPascaFakultasStandar1Form extends S7DokumentasiPascaFakultasStandar1
{
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiPascaFakultasStandar1();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiPascaFakultasStandar1::find()->select('dokumen')->all();
