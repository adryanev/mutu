...
class S7DokumentasiInstitusiStandar2Form extends S7DokumentasiInstitusiStandar2
{
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiInstitusiStandar2();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiInstitusiStandar2::find()->select('dokumen')->all();
