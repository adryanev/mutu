...
class S7DokumentasiPascaProdiStandar2Form extends S7DokumentasiPascaProdiStandar2 {
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiPascaProdiStandar2();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiPascaProdiStandar2::find()->select('dokumen')->all();
