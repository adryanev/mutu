...
class S7DokumentasiPascaProdiStandar5Form extends S7DokumentasiPascaProdiStandar5 {
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiPascaProdiStandar5();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiPascaProdiStandar5::find()->select('dokumen')->all();
