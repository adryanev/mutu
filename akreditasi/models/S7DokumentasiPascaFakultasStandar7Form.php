...
class S7DokumentasiPascaFakultasStandar7Form extends S7DokumentasiPascaFakultasStandar7
{
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiPascaFakultasStandar7();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiPascaFakultasStandar7::find()->select('dokumen')->all();
