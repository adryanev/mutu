...
class S7DokumentasiPascaFakultasStandar2Form extends S7DokumentasiPascaFakultasStandar2
{
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiPascaFakultasStandar2();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiPascaFakultasStandar2::find()->select('dokumen')->all();
