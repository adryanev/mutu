...
class S7DokumentasiS1FakultasStandar3Form extends S7DokumentasiS1FakultasStandar3
{
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiS1FakultasStandar3();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiS1FakultasStandar3::find()->select('dokumen')->all();
