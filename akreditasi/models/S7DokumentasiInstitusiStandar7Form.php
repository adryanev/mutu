...
class S7DokumentasiInstitusiStandar7Form extends S7DokumentasiInstitusiStandar7
{
    ...
    public function uploadDokumen($id){
        ...
        if($this->validate()){
            $this->_dokumenDokumentasi = new S7DokumentasiInstitusiStandar7();
            $fileName = $this->dokumenDokumentasi->getBaseName().'.'.$this->dokumenDokumentasi->getExtension();

            $model = S7DokumentasiInstitusiStandar7::find()->select('dokumen')->all();
